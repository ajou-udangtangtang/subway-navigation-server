...
> **오늘**: 14:30 세류역 4명 현장 통합 테스트 — 출발 전 서버·ngrok 가동 필수
> 최근 머지: PR #3 (화경 마스킹 거리 KNN, dropout 환경 21%→87.5%)
...
### 🟡 현재 임계 경로 — **현장 통합 테스트 + 잔여 이슈 해결**

5/26 부터 앱↔서버 통합 테스트 시작됨. 5/31 박경찬 현장 단독 테스트 결과 *"얼추 되는데 정합성 약함"*. **2026-06-03 14:30 세류역에서 4명 모여 현장 통합 테스트** 진행.

**오늘 미팅 안건** (요약):
1. **위치 정합성 약함** — 비슷한 위치(출입구·1층계단·개찰구·승강장) 혼동. 데이터 추가 수집 또는 앱 회복 로직 강화
2. **앱 UI 자동 진행** — 방향 맞으면 자동 다음 단계로 가야 하는데 버튼 눌러야 (5/31 발견)
3. **회복 로직 + 평균값 동시 적용 검토** (박경찬 5/31)
4. **Claude 결제 만료 ~6/6** — 1개월 추가 결제 (6/2 박경찬 제안)
5. **시연 시나리오 합의**

상세 흐름: `worklog/2026-06-03-integration-issues-and-onsite-prep.md`
...
본인 영역(서버)은 안정. 모든 잔여 이슈는 **앱(최수빈) 또는 데이터(현장 측위 정확도)** 영역.

| 영역 | 이슈 |
|---|---|
| 측위 정확도 | 5/27 *"제멋대로"*, 5/31 *"얼추 되는데 약함"* — 비슷한 위치 혼동 |
| 앱 UI | 방향 맞아도 자동 진행 안 됨 (5/31), TTS 작동 (5/27 해결) |
| 앱 로직 | 5회 중 3회 + 평균 RSSI + 상위 20 AP (v4, 5/29) — 동시 적용 OK 여부 확인 |
