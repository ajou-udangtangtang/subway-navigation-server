...
>
> 마지막 갱신: **2026-06-07**
> 다음 마일스톤: **2026-06-12 최종 발표 (D-5) / 2026-06-19 최종 제출**
> 최근 사건: **6/3 세류역 4명 현장 통합 테스트 완료** — 전체 경로 진행 확인(출입구→…→엘리베이터). A안(앱 passed) 채택, 스무더·B안 폐기, stairs_mid 제거 (커밋 6개 6/7 푸시 완료)
...
- **대상 역**: 1개 역 (9 노드: 출입구·개찰구·계단·승강장 등 — floor1_hall·stairs_mid 는 6/3 제거됨)
...
| MySQL DB + KNN(마스킹 거리) + Wi-Fi 필터 | 김화경 | `scripts/subway_nav_full.sql`, `subway_server/core/knn.py`(PR #3), `wifi_filter.py` |
| Flask 서버 + 3 API + Swagger UI | 본인 | `subway_server/`, `app.py` |
| 단위 + 통합 + E2E 테스트 79개 (66 + 13) | 본인 | `tests/` |
| ngrok 호스팅 + 정합성 점검 + 검증 스크립트 | 본인 | `sporty-press-unfeeling.ngrok-free.dev`, `scripts/verify_api_with_real_data.py` |
| 안드로이드 앱 v4 (위치 정합성·진동 반전·센서 리스너 수정) | 최수빈 | `csb2000/subway-navigation-app` (v4 브랜치) |

### ✅ 6/3 세류역 현장 통합 테스트 결과 (worklog `2026-06-03-4/5/6` 상세)

4명 현장 테스트에서 측위 이슈를 실데이터로 진단하고 당일 서버 대응 3건을 시도·검증:

| 시도 | 결과 |
|---|---|
| `/locate` `exclude`/`passed` 옵션 (A안 — 앱이 지나온 노드 전송) | ✅ **채택** — 전체 경로 진행 정상 확인 (출입구→개찰구→1층계단→지하계단→엘리베이터) |
| 서버 자동추론 `nav_progress` (B안) | ❌ 폐기 — 출발 노드까지 제외하는 결함. `AUTO_EXCLUDE_PASSED` 기본 OFF |
| 경로 스무딩 `nav_smoother` (뒤로 금지) | ❌ 되돌림 — forward-lock 부작용 (잘못 전진 시 자가 교정 불가). `SMOOTH_LOCATE` 기본 OFF |
| **stairs_mid 노드 제거** (계단 조기 인식 완화) | ✅ 그래프 JSON + 라이브 DB 반영, 79 테스트 그린. 시드 SQL 정식 반영은 화경 몫 |

**결론**: 측위 보정은 서버 후처리(스무더·자동추론) 다 부작용이 큼 → **앱 누적 passed 단독이 가장 안정**.

### 🟡 현재 임계 경로 — **발표 준비 (6/12, D-5)**

서버는 안정·코드 변경 사실상 종료. 남은 건 시연 시나리오 + 발표 자료 + 소폭 잔여 이슈.

**현재 호스팅 상태** (노트북 켜져 있을 때):
- ngrok 고정 주소: `https://sporty-press-unfeeling.ngrok-free.dev`
- Flask 포트 **5001** (5000 은 macOS AirPlay 충돌)
- 로컬 Docker MySQL `subway_nav` (포트 6306, root/lowell), fingerprints 455행 (stairs_mid 제거 후)
...
### ⚠️ 운영 합의 (5/29~)
- 본인·화경 외출 시 서버 다운 위험 → 박경찬·최수빈 테스트 **1~2시간 전 사전 통보** 필수
- 본인은 평일 어렵고 주말 외출 잦음. 사전 통보만 받으면 외부에서도 서버 켤 수 있음 (가능하면 화경에게도 부탁)

### 🚨 잔여 이슈 (06-07 기준)

| 영역 | 이슈 | 비고 |
|---|---|---|
| 앱 (최수빈) | passed 를 **직전 1개만** 전송 → 누적 전체 리스트로 바꾸면 개찰구→계단 구간 출입구 튐(~30초) 해소 | 사용자 결정: 일단 패스 |
| 화경 | stairs_mid 제거 시드 SQL(`subway_nav_full.sql`) 정식 반영 + `knn_estimate(exclude=...)` 정식 추가 → 임시 `locate_filter.py` 제거 | 사용자 결정: 공유 생략 |
| 측위 | 계단 '조기 전진' — Wi-Fi 한계로 잔존. 안내가 "내려가세요"라 시연 무해. 근본책(모션모델/데이터)은 발표 후 | |
| 인프라 | 외출 시 서버 다운 (사전 통보로 운영) | |

### ⏳ 6/12 발표까지 남은 작업 (D-5)

1. ~~서버 호스팅~~ ✅ / ~~앱↔서버 통합~~ ✅ / ~~현장 통합 테스트~~ ✅ (6/3)
2. 시연 시나리오 합의 (출발지·목적지) — 6/3 미팅 결정 사항 미기록, 단톡방 확인
3. 최종 발표 자료 + 영상
4. (시연 당일) 서버·ngrok·Docker MySQL 가동 체크리스트
...
```bash
pytest tests/unit tests/integration -q   # 빠른 로컬, 66 tests
...
### Q. 다음에 뭐 하면 됨?
§3의 *"⏳ 6/12 발표까지 남은 작업"* 참조. 시연 시나리오 + 발표 자료가 임계 경로.
